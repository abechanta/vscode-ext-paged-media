...
class ViewerPanel {
	...
	private _update() {
		try {
			...
			const htmlBodyContent = this._getHtmlBodyContent();

			this._panel.webview.html = this._getHtmlContent({
				pagedjsUri: pagedjsUri,
				scriptUri: scriptUri,
				styleUri: styleUri,
	...
	private _getHtmlBodyContent() {
		if (!vscode.window.activeTextEditor) {
			throw new Error("No document loaded.");
		}
		const document = vscode.window.activeTextEditor.document;
        if (!document.languageId.match("html")) {
			throw new Error(`Language "${document.languageId}" is not acceptable.`);
        }
