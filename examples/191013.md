...
＜この本のタイトル＞<br />Vol.1 {#title .red}

＜この{本|ブック}の{著者|オーサー}＞ {#author}

＜この本の版数＞ 版　　＜この本の製作者＞ 発行 {#issue}
...
TODO:
- テキスト図表に対応する
- upstream #140: counter-reset/counter-increment を複数指定に対応する／content(before) に対応する／柱にチャプター番号を埋め込む／図表参照に図表番号を埋め込む
- upstream #139: break-after: avoid を機能させる
- page-break-inside: avoid を機能させる
- 注釈を各ページに埋め込む
- overflow を修正する

![](SAMPLE-TITLE.md rel=content)

:::

![](SAMPLE-1.md rel=content)
...
![link7](../vscode-command-viewer/media/bierner.markdown-mermaid.svg "（左） 何かの図７のキャプション１"){style="height: 40px;"}
![link7](./media/bierner.markdown-mermaid.svg "（右） 何かの図７のキャプション２")
{.figures}

左：何かの図７のキャプション１／右：何かの図７のキャプション２２２２２２２２２{.caption #fig1 style=" open-quote='***';"}

インライン画像は ![](../vscode-command-viewer/media/bierner.markdown-mermaid.svg) こんな感じになります。

この様子を [](#fig8) に示します。

![fig8](../vscode-command-viewer/media/bierner.markdown-mermaid.svg){.shadow}
{.figures}

なにかを説明する図{.caption #fig8}

詳しくは[後述][chapter-sprite]します。
[][chapter-sprite] 参照。

{スプライト|すぷらいと}として2D画像を表示する場合、表示したい画像をそれぞれ別のテクスチャとして持つこともできますが、テクスチャの切り替えには時間がかかるため、通常はできるだけ少ない枚数のテクスチャにまとめてしまいます。
そうすると、元の画像の範囲が分かるようなデータが必要になってきます。
このデータは「セル(Cell)」とか「チップ(Chip)」とか呼ばれます。今回はセルと呼ぶことにします<span class="fn">これは Paged Media Level 3 に基づく注釈です。</span>。

まずは、`Src`フォルダに`Sprite.h`ファイルを追加してください。[^1]

[^1]: これは markdown-it-footnote-conventional に基づく注釈です。
...
# スプライト描画クラス {data-alias=chapter-sprite}
